            ...
            <div class="team-body">
                @if($head)
                    <div class="staff-row bg-warning" style="border-left: 3px solid #f39c12; background-color: #fcf8e3; border-radius: 3px;">
                        <div>
                            <span style="font-size: 15px; font-weight: bold; color: #8a6d3b;">{{ $head->user->first_name }} {{ $head->user->last_name }}</span><br>
                            <small style="color: #8a6d3b;">Username: {{ $head->user->username }} | EMP No: {{ $head->user->employee_num ?? 'N/A' }}</small>
                        </div>
