    ...
    if ($('.sidebar-menu').length) {
        ...
        var storeMenu = '<li class="treeview ' + (isStoreActive ? 'active' : '') + '" id="gov-store-parent-menu">' +
            ...
            '<ul class="treeview-menu">' +
                ...
                '<li class="' + (isCatalogActive ? 'active' : '') + '"><a href="<?php echo e(route("gov.requests.catalog")); ?>"><i class="fas fa-store fa-fw"></i> Browse Catalog</a></li>' +
                '<li class="' + (isMyRequestsActive ? 'active' : '') + '"><a href="<?php echo e(route("gov.requests.user.index")); ?>"><i class="fas fa-clipboard-list fa-fw"></i> My Requests</a></li>';

        // 2. Inject Restricted "Store Operations" section strictly to authorized staff
        <?php if(auth()->user()->isSuperUser() || auth()->user()->hasAccess('admin') || auth()->user()->hasAccess('superuser')): ?>
            storeMenu += '<li class="header" style="padding: 5px 15px; font-size: 10px; color: #72afd2; background: #1a2226; letter-spacing: 1px;">STORE OPERATIONS</li>' +
                '<li class="' + (isApprovalsActive ? 'active' : '') + '"><a href="<?php echo e(route("gov.requests.admin.index")); ?>"><i class="fas fa-clipboard-check fa-fw"></i> Gov Approvals</a></li>' +
                '<li class="' + (isFulfillmentActive ? 'active' : '') + '"><a href="<?php echo e(route("gov.requests.fulfillment.index")); ?>"><i class="fas fa-shipping-fast fa-fw"></i> Fulfillment Queue</a></li>' +
                '<li class="' + (isPoliciesActive ? 'active' : '') + '"><a href="<?php echo e(route("gov.requests.admin.policies.index")); ?>"><i class="fas fa-tags fa-fw"></i> Category Policies</a></li>';
        <?php endif; ?>
...
</script><?php /**PATH D:\git repo\asset-store-bd\packages\gov-store\custom-requests\src\Providers/../resources/views/hooks/menu-injection.blade.php ENDPATH**/ ?>